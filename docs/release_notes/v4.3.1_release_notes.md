# 🚀 v4.3.1 – SoM-Centric Pipeline with Extended Metrics & Dissertation Final
...
This release introduces the **SoM-centric ML pipeline** with PhD-level evaluation metrics, naïve baselines comparison, and finalized dissertation figures. The pipeline now targets **State-of-Mind (SoM) labels** from ecological momentary assessment (EMA) rather than PBSI-derived labels, providing clinical validation alignment.

**Key improvements:**
- ✅ SoM-centric target variables (77 labeled days from EMA diary)
- ✅ PhD-level metrics module (`ml_metrics_extended.py`)
- ✅ Naïve baselines: majority-class, stratified-random, persistence
- ✅ Per-class metrics with confusion matrix aggregation
- ✅ Extended HRV features (SDNN mean/median/min/max)
- ✅ Medication features integration (med_any, med_event_count, med_dose_total)
- ✅ 13 dissertation figures regenerated with improved visualizations
...
|-------|----------|--------------|---------|
| **XGBoost** | **0.63 ± 0.19** | **0.63 ± 0.19** | **0.27 ± 0.37** |
| GradientBoosting | 0.57 ± 0.14 | 0.65 ± 0.21 | 0.18 ± 0.27 |
| LogisticRegression | 0.47 ± 0.17 | 0.57 ± 0.19 | 0.08 ± 0.26 |
| RandomForest | 0.46 ± 0.02 | 0.50 ± 0.00 | 0.00 ± 0.00 |

**Baseline comparison (XGBoost vs naïve):**

| Method | F1-Macro | Balanced Acc | Cohen κ |
|--------|----------|--------------|---------|
...
| Model | F1-Macro | Balanced Acc | Cohen κ |
|-------|----------|--------------|---------|
| **CNN_LSTM** | **0.53 ± 0.16** | **0.63 ± 0.18** | **0.16 ± 0.26** |
| **LSTM** | **0.52 ± 0.23** | **0.65 ± 0.23** | N/A |
| Conv1D | 0.44 ± 0.03 | 0.54 ± 0.12 | -0.02 ± 0.04 |
| GRU | 0.43 ± 0.22 | 0.57 ± 0.19 | 0.07 ± 0.29 |
...
3. **Export Structure:**
   ...
   │   ├── baseline_comparison_binary.csv
   │   └── baseline_comparison_seq_som_binary.csv
   ├── confusion_matrices/
   │   ├── cm_XGBoost_binary.json
   │   ├── cm_CNN_LSTM_seq_som_binary.json
   │   └── cm_GRU_seq_som_binary.json
   └── per_class/
       ├── per_class_XGBoost_binary.csv
       └── per_class_CNN_LSTM_seq_som_binary.csv
   ```
...
- `docs/ETL_ARCHITECTURE_COMPLETE.md` – Full pipeline architecture
- `docs/PHASE3_ENRICHED_GLOBAL_ARCHITECTURE.md` – Enriched features design
- `docs/QUICK_REFERENCE_ETL.md` – ETL quick reference guide
...
### New Files
- `src/etl/ml_metrics_extended.py` – PhD-level evaluation metrics
- `results/metrics/P000001/2025-12-09/*` – Exported metrics (CSV/JSON)

### Modified Files
- `src/etl/ml6_extended.py` – Extended metrics integration
- `src/etl/ml7_extended.py` – Extended metrics integration
- `scripts/generate_dissertation_figures.py` – Updated to snapshot 2025-12-09
- `scripts/run_full_pipeline.py` – MICE imputation fix
- `src/labels/build_pbsi.py` – Segment handling fix
- `notebooks/generate_dissertation_figures.ipynb` – Updated paths
