...
**v4.1.0** represents a major consolidation milestone, completing **Fase 3** of the ETL→Modeling pipeline with production-ready analytics, unified extract infrastructure for Apple/Zepp variants, and deterministic CLI entrypoints for cardio/sleep/activity domain seeds.
...
**Key achievement:** End-to-end reproducibility from raw device exports → normalized domain datasets → feature engineering, with full audit trail via manifest-based QC.
...
- **Unified Extract Orchestration** (`5dc0192`):
...
- **Snapshot Path Normalization** (`65ba18d`):

  - Removed redundant `snapshots/` directory nesting.
  - Canonical path: `data/etl/<PID>/<YYYY-MM-DD>/` (**Closes #9**).
  - Migration helper included for legacy layouts.

- **Cardio Output Fix** (`6a7768a`):

  - Corrected PID extraction from snapshot directory.
  - Cardio outputs now write to correct `data/etl/<PID>/<SNAP>/joined/` (**Closes #10**).
...
### 🧪 Fase 3 Analytics Pipeline (Production-Ready)

- **Join Coalescence & QC** (`89e802e`):
...
- **CLI Domain Entrypoints** (`6184580`):

  - `etl_runner activity` — seed activity features from extracted data.
  - `etl_runner cardio` — join cardiovascular metrics with QC.
  - `etl_runner sleep` — aggregate sleep intervals (stub).
  - Dry-run mode with idempotence checks.

- **Notebook Integration** (`c84d934`):
...
- **ETL Namespace & Makefile Modernization** (`8ac25db`):
...
- **Optional Progress Visualization** (`de43a95`):
...
- **Activity Import Robustness** (`4d43e33`):
...
- **CLI Dry-Run Semantics** (`e7b9881`):
  - Harmonized exit codes: dry-run=0, empty real run=2.
  - Better CI/automation compatibility.

### 📊 Data Snapshot Date Resolution (**Closes #13**)

- **Manifest-Based Provenance** (`5dc0192`):
...
- **NB1_EDA_MIGRATION.md** (`c517d89`):
  - Consolidation report documenting Fase 1→3 analytics workflow.
...
- ✅ Cardio/sleep seed tests pass (atomic writes, manifest validation).
- ✅ Extract QC validates against 30-day sample from P000001.
- ✅ Zepp parsing tested with AutoExport CSVs and export zips.
- ✅ Cross-platform Makefile validated on Windows (bash.exe) and Linux.
...
- **v4.2.0:** Extend idempotence guarantees to Zepp and iOS stages (B1–B3, Z1–Z3).
