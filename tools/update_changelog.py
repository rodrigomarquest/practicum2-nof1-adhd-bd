...
def get_previous_tag() -> str | None:
    try:
        ...
        out = subprocess.check_output(['git', 'describe', '--tags', '--abbrev=0', 'HEAD^'], stderr=subprocess.DEVNULL, text=True)
        return out.strip()
    except Exception:
        try:
            # fallback to previous tag reachable
            out = subprocess.check_output(['git', 'describe', '--tags', '--abbrev=0'], stderr=subprocess.DEVNULL, text=True)
            return out.strip()
        except Exception:
            return None
...
def main(argv=None) -> int:
    ...
    mapping = {
        ...
        'ADDED': '--', 'CHANGED': '--', 'FIXED': '--', 'TESTS': '--', 'NOTES': '--'
    }
    ...
    if f'## [{args.version}]' in existing or args.version in existing.splitlines()[0:30]:
    ...
    compare_link = f'[{args.version}]: {repo_root}/compare/{prev or ''}...{args.version}\n'
