...
CONTENT = r'''"""Cardio feature derivation (functional minimal).
"""
import math
import os
from typing import Optional
...
def derive_hr_features(hr: Optional[pd.DataFrame], sleep, segments, cfg: CardioCfg) -> pd.DataFrame:
    ...
    if "awake" not in agg_period.columns:
        agg_period["awake"] = np.nan
    if "night" not in agg_period.columns:
        agg_period["night"] = np.nan
...
def derive_hrv_features(hrv: Optional[pd.DataFrame], hr, sleep, segments, cfg: CardioCfg) -> pd.DataFrame:
...
def derive_circadian_features(hr: Optional[pd.DataFrame], segments, cfg: CardioCfg) -> pd.DataFrame:
...
def main():
    target = Path('etl_modules/cardiovascular/cardio_features.py')
