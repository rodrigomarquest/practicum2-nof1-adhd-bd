...
import os, argparse, io, csv, codecs
...
def _concat_nonempty(dfs: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    clean = [d for d in dfs if isinstance(d, pd.DataFrame) and not d.empty and d.shape[1] > 0]
    if not clean:
        return pd.DataFrame()
...
def _read_csv_robust_text(text: str) -> pd.DataFrame:
    ...
    try:
        return pd.read_csv(io.StringIO(text), sep=(sep or None), engine="python", on_bad_lines="skip")
    except Exception:
        return pd.read_csv(io.StringIO(text), sep="\t", engine="python", on_bad_lines="skip")

def _read_csv_robust_from_zip(zf, member_name: str, password: str | None) -> pd.DataFrame:
    # tenta sem senha e depois com pwd=
    try:
        with zf.open(member_name, "r") as fh:
            data = fh.read()
    except Exception:
        with zf.open(member_name, "r",
                     pwd=(password.encode("utf-8") if password else None)) as fh:
            data = fh.read()
...
def _read_zip_table(zip_path: Path, password: str | None, folder_prefix: str) -> pd.DataFrame:
    """Concatena todos CSVs dentro de uma pasta (ex.: 'SLEEP/') de um ZIP."""
    zf = _open_zip_any(zip_path, password)
    try:
        names = zf.namelist()
        sel = [n for n in names if _norm_posix_lower(n).startswith(_norm_posix_lower(folder_prefix))]
...
def _to_local_date(series_utc, cutover_str: str, tz_before: str, tz_after: str) -> pd.Series:
    """
...
def append_manifest(pid: str, outroot: Path, zip_path: Path, export_id: str,
                    categories: list[str], df_coverage: dict):
    import csv as _csv
...
def parse_args():
    ...
    ap.add_argument("--outdir-root", required=True, help="Root dir for processed outputs (per export_id)")
    ap.add_argument("--password", default=None, help="ZIP password (fallback ZEPP_ZIP_PASSWORD)")
    # cutover Brazil -> Ireland
    ap.add_argument("--cutover", required=True, help="Cutover date YYYY-MM-DD (BR→IE)")
    ap.add_argument("--tz_before", required=True, help="IANA TZ before cutover, e.g., America/Sao_Paulo")
    ap.add_argument("--tz_after", required=True, help="IANA TZ after cutover, e.g., Europe/Dublin")
    ap.add_argument("--export-id", default=None, help="Override export_id (default: derived from input)")
    ap.add_argument("--participant", default="P000001", help="Participant ID (for manifests)")
...
def main():
    ...
    is_dir = src.is_dir()
    read_table = (lambda prefix: _read_dir_table(src, prefix)) if is_dir else (lambda prefix: _read_zip_table(src, zip_password, prefix))

    # ---------------- HEARTRATE + HEARTRATE_AUTO ----------------
    hr   = read_table("HEARTRATE/")
    hra  = read_table("HEARTRATE_AUTO/")
    hr_all = pd.concat([hr, hra], ignore_index=True) if (not hr.empty or not hra.empty) else pd.DataFrame()

    hr_daily = pd.DataFrame(columns=["date","zepp_hr_mean","zepp_hr_median","zepp_hr_p95"])
    if not hr_all.empty:
        ...
        if ts_col and val_col:
            hr_all["date"] = _to_local_date(hr_all[ts_col], args.cutover, args.tz_before, args.tz_after)
            hr_all[val_col] = pd.to_numeric(hr_all[val_col], errors="coerce")
            g = hr_all.groupby("date", dropna=False)[val_col]
            hr_daily = g.agg(zepp_hr_mean="mean",
                             zepp_hr_median="median",
                             zepp_hr_p95=lambda s: s.quantile(0.95)).reset_index()
    ...
    if not sleep.empty:
        ...
        if all(k in cols for k in ["date","deepsleeptime","shallowsleeptime"]) and ("remtime" in cols or "naps" in cols):
            date_col = cols["date"]
            dcol = cols["deepsleeptime"]; scol = cols["shallowsleeptime"]
            rcol = cols.get("remtime");   ncol = cols.get("naps")
            for c in [dcol, scol, rcol, ncol]:
                if c and c in sleep.columns:
                    sleep[c] = pd.to_numeric(sleep[c], errors="coerce").fillna(0)
            total_min = sum([sleep[dcol], sleep[scol]] + ([sleep[rcol]] if rcol else []) + ([sleep[ncol]] if ncol else []))
            sleep_daily = (pd.DataFrame({"date": sleep[date_col].astype("string"),
                                         "zepp_sleep_minutes": total_min})
                           .groupby("date", dropna=False)["zepp_sleep_minutes"]
                           .sum(min_count=1).reset_index())
        else:
            ts_col  = _maybe_col(sleep, "timestamp","time","dateTime","start","startTime","date")
            dur_col = _maybe_col(sleep, "duration","sleep_duration","minutes","sleepMinutes")
            if ts_col and dur_col:
                sleep["date"] = _to_local_date(sleep[ts_col], args.cutover, args.tz_before, args.tz_after)
                mins = pd.to_numeric(sleep[dur_col], errors="coerce")
                sleep_daily = sleep.groupby("date", dropna=False)[mins.name].sum().reset_index()
                sleep_daily = sleep_daily.rename(columns={mins.name: "zepp_sleep_minutes"})
    ...
    activity_daily = pd.DataFrame(columns=["date","zepp_steps","zepp_calories","zepp_active_minutes"])
    if not act.empty:
        ...
        if ts_col and steps_col:
            act["date"] = _to_local_date(act[ts_col], args.cutover, args.tz_before, args.tz_after)
            daily = act.groupby("date", dropna=False).agg({steps_col: "sum", **({cal_col: "sum"} if cal_col else {})}).reset_index()
            daily = daily.rename(columns={steps_col: "zepp_steps"})
            if cal_col: daily = daily.rename(columns={cal_col: "zepp_calories"})
            else:       daily["zepp_calories"] = 0
            activity_daily = daily
    if not actm.empty:
        ts_col = _maybe_col(actm, "timestamp","time","dateTime","startTime","date")
        dur_col = _maybe_col(actm, "duration","minutes","active_minutes","activityMinutes")
        if ts_col and dur_col:
            actm["date"] = _to_local_date(actm[ts_col], args.cutover, args.tz_before, args.tz_after)
            tmp = actm.groupby("date", dropna=False)[dur_col].sum().reset_index().rename(columns={dur_col:"zepp_active_minutes"})
            activity_daily = activity_daily.merge(tmp, on="date", how="outer") if not activity_daily.empty else tmp
    ...
    if not body.empty:
        ts_col = _maybe_col(body, "timestamp","time","dateTime","measureTime","startTime","date")
        ...
        if ts_col and (w_col or bf_col):
            body["date"] = _to_local_date(body[ts_col], args.cutover, args.tz_before, args.tz_after)
            agg = {}
            if w_col:  agg[w_col]  = "mean"
            if bf_col: agg[bf_col] = "mean"
            bd = body.groupby("date", dropna=False).agg(agg).reset_index()
            if w_col:  bd = bd.rename(columns={w_col:"zepp_weight_kg"})
            if bf_col: bd = bd.rename(columns={bf_col:"zepp_bodyfat_pct"})
            body_daily = bd
    safe_write_csv(body_daily, processed_root / "zepp_body_daily.csv")

    # ---------------- HEALTH_DATA ----------------
    hdata = read_table("HEALTH_DATA/")
    health_daily = pd.DataFrame(columns=["date","zepp_spo2_mean","zepp_temp_mean","zepp_stress_mean"])
    if not hdata.empty:
        ts_col = _maybe_col(hdata, "timestamp","time","dateTime","startTime","measureTime","date")
        spo2   = _maybe_col(hdata, "spo2","blood_oxygen","oxygensaturation","saturation")
        temp   = _maybe_col(hdata, "temp","temperature","skin_temp","skin_temperature","body_temperature")
        stress = _maybe_col(hdata, "stress","stress_score","mental_stress")
        if ts_col:
            hdata["date"] = _to_local_date(hdata[ts_col], args.cutover, args.tz_before, args.tz_after)
            pieces = []
            if spo2:
                pieces.append(hdata.groupby("date", dropna=False)[spo2].mean(numeric_only=True).reset_index().rename(columns={spo2:"zepp_spo2_mean"}))
            if temp:
                pieces.append(hdata.groupby("date", dropna=False)[temp].mean(numeric_only=True).reset_index().rename(columns={temp:"zepp_temp_mean"}))
            if stress:
                pieces.append(hdata.groupby("date", dropna=False)[stress].mean(numeric_only=True).reset_index().rename(columns={stress:"zepp_stress_mean"}))
            if pieces:
                base = pieces[0]
                for p in pieces[1:]:
    ...
    dfs = [d for d in (hr_daily, sleep_daily, activity_daily, body_daily, health_daily) if (d is not None and not d.empty)]
    if dfs:
        daily = dfs[0]
        for d in dfs[1:]:
    ...
    # ---------- rebuild _latest ----------
    for fname in ["zepp_sleep_daily.csv","zepp_hr_daily.csv","zepp_health_daily.csv","zepp_body_daily.csv","zepp_activity_daily.csv","zepp_daily_features.csv"]:
