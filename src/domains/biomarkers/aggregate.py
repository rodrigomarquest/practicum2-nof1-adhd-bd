...
def _load_csv(path: Optional[str]) -> pd.DataFrame:
    """Load CSV file safely and normalize column names."""
    if not path:
        return pd.DataFrame()
    try:
        # Use Python engine for more lenient parsing (handles embedded newlines in JSON fields)
        # Skip bad lines to avoid tokenization errors
        df = pd.read_csv(path, engine='python', on_bad_lines='skip')
        # Normalize Zepp camelCase column names to snake_case
        df = _normalize_zepp_columns(df)
        # Combine date and time into timestamp if needed
        df = _combine_date_time(df)
        return df
...
def _combine_date_time(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'date' in df.columns and 'time' in df.columns and 'timestamp' not in df.columns:
        try:
            ...
            logger.debug(f"Could not combine date/time into timestamp: {e}")
    return df
...
def _normalize_zepp_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - REMTime → rem_minutes
    - naps → naps (already correct)
    
    Note:
    - 'steps' column is left as-is (used by multiple functions with different expectations)
    - 'time' column is kept as-is; it will be combined with 'date' to create 'timestamp'
    """
    column_mapping = {
        'heartRate': 'heart_rate',
        'deepSleepTime': 'deep_minutes',
