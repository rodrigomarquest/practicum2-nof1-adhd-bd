...
def _normalize_dt_with_cutover(
    ...
    target_tz = (
        tz.gettz(tz_before) if ts_utc.date() < cutover_date else tz.gettz(tz_after)
    )
    return ts_utc.tz_convert(target_tz)


def extract_apple_screen_time(
    ...
    for _, elem in ctx:
        ...
        if "screentime" not in rtype:
            elem.clear()
            continue
        start_dt = _normalize_dt_with_cutover(
            elem.attrib.get("startDate"), cutover_str, tz_before, tz_after
        )
        end_dt = _normalize_dt_with_cutover(
            elem.attrib.get("endDate"), cutover_str, tz_before, tz_after
        )
        seconds = None
    ...
    df_usage = pd.DataFrame(
        {
            ...
            "screen_time_min": np.array(list(agg_seconds.values()), dtype="float64")
            / 60.0,
        }
    ).sort_values("date")
