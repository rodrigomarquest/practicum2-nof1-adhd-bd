...
def analyze_file(p: Path):
    ...
    try:
        ...
        info['null_counts'] = ';'.join(f"{c}:{int(df[c].isna().sum())}" for c in df.columns)
        ...
        if primary:
            try:
                ...
                tz_indicators = any((('+' in v and ':' in v) or v.endswith('Z')) for v in raw_sample)
...
def run_consistency_checks(snapshot_dir: Path, out_csv: Path):
    ...
    for scope, a_name, b_name, tol in pairs:
        try:
            ...
            # status rules
            threshold = 60 if snapshot_length_days >= 60 else max(1, snapshot_length_days)
            ...
            csv_rows.append({
                ...
                'a_file': str(a_file),
                'b_file': str(b_file),
                'value': json.dumps({'median_diff': median_diff, 'p10': p10, 'p50': p50, 'p90': p90, 'n': n}),
                'status': status,
    ...
    keys = ['check_id', 'check_type', 'scope', 'metric', 'a_file', 'b_file', 'value', 'status', 'details']
...
def save_coverage_plot(report_csv: Path, out_png: Path):
    ...
    if 'coverage_days' in df.columns:
        df['coverage_days'] = pd.to_numeric(df['coverage_days'], errors='coerce').fillna(0)
    else:
        df['coverage_days'] = 0
...
def update_markdown(summary_md: Path, checks_csv: Path, coverage_png: Path, report_csv: Path):
    ...
    if 'Overview' not in txt:
        ...
        n_files = len(rpt)
        min_dates = rpt['min_date'].dropna().tolist() if 'min_date' in rpt.columns else []
        max_dates = rpt['max_date'].dropna().tolist() if 'max_date' in rpt.columns else []
    ...
    for i, r in checks.iterrows():
        ...
        try:
            ...
            median = ''
        rows.append(f"| {r['scope']} | {r['a_file']} | {r['b_file']} | {r.get('details','')} | {median} | {r['status']} |")

    checks_section = '\n## Apple vs Zepp Consistency Checks\n\n'
    checks_section += '| scope | a_file | b_file | days_overlap | median_diff | status |\n'
    checks_section += '|---|---:|---:|---:|---:|---:|\n'
    checks_section += '\n'.join(rows) + '\n\n'
...
def main(argv=None):
    ...
    p.add_argument('--snapshot', default='data_ai/P000001/snapshots/2025-09-29')
    ...
    with summary.open('w', encoding='utf-8') as fh:
        ...
        for r in rows:
            fh.write(f"- {r.get('file')}: rows={r.get('rows','?')} cols={r.get('ncols','?')}\n")
