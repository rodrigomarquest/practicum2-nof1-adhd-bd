#!/usr/bin/env python3
"""Fuse Apple and Zepp dailies into AI features with provenance.

Produces fused_* columns in the AI features CSV and writes
`xsource_fusion_manifest.json` with per-day chosen source and QC flags.
Deterministic, atomic, idempotent.
"""
...
def write_csv_rows(path: Path, rows: List[Dict[str, Any]], headers: List[str]):
    ...
    with tmp.open("w", newline="", encoding="utf-8") as fh:
        ...
        for r in rows:
            writer.writerow({
                k: ("" if r.get(k) is None or (isinstance(r.get(k), float) and math.isnan(r.get(k))) else r.get(k))
                for k in headers
            })
    tmp.replace(path)


def csv_has_column(path: Path, col: str) -> bool:
    """Return True if the CSV at path contains column `col` in its header.

    This avoids importing pandas for a quick header check.
    """
    if not path.exists():
        return False
    with path.open(newline="", encoding="utf-8") as fh:
...
def main(argv=None):
    ...
    per_metric_manifest: Dict[str, Any] = {}
    ...
    # For later use build a simple metrics list (metric_key, cfg, fused_col_name)
    metrics = [(mk, cfg, f"fused_{mk}") for mk, cfg in metric_map.items()]

    # for determinism, we will produce a new ai table preserving original rows and appending fused cols
    fused_headers = [fused for _, _, fused in metrics]

    # initialize manifest structure
    manifest: Dict[str, Any] = {"generated_at": datetime.utcnow().isoformat() + "Z", "metrics": {}}
            ...
            if block_fusion:
                ...
                if fusion_policy == "quality_weighted_mean":
                    ...
                    for cc in ["count", "n", "rows"]:
                        if apple_file.exists() and csv_has_column(apple_file, cc):
                            a_count = read_csv_map(apple_file, [cc]).get(d, {}).get(cc)
                            break
                    for cc in ["count", "n", "rows"]:
                        if zepp_file.exists() and csv_has_column(zepp_file, cc):
