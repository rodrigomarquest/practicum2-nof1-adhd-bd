...
def safe_rm_tree(target: Path, repo_root: Path, dry_run: bool = False) -> list[Path]:
    """Remove target tree only if it is inside repo_root. Returns True if removed (or would be removed in dry-run).

    When dry_run=True, nothing is deleted; instead the function prints what would be removed and returns True if the
    target exists and is inside the repo.
    """
    try:
...
def prompt_yes(question: str, default: bool = False) -> bool:
    """Prompt the user for a yes/no and return True for yes."""
    try:
...
def prompt_exact(question: str, token: str) -> bool:
    """Prompt user and require exact token match (case-sensitive)."""
    try:
        resp = input(question)
    except EOFError:
        return False
...
def infer_pid(snapshot_path: Path) -> str | None:
    """Infer PID from a snapshot path of form data_ai/<PID>/snapshots/..."""
