...
try:
    ...
    def ks_2samp(a, b):
        # fallback when scipy isn't available during lightweight import checks
        return float('nan'), float('nan')


def load_feature_series(root: Path, metric_name: str) -> Dict[str, float]:
    """Find CSVs under `root` that contain `metric_name` and return a dict date -> value.

    The function is intentionally tolerant of different CSV column layouts used across
    processed apple/zepp files.
    """
...
def adwin_like_flag(a_vals, b_vals, delta=0.002):
    """Simple relative-mean-shift detector used as a cheap proxy for ADWIN-like changes.
    Returns True when the relative mean shift exceeds `delta`.
    """
