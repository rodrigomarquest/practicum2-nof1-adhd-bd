...
# Patterns to search for: data/etl/<pid>/normalized, data/etl/<pid>/processed, data/etl/<pid>/joined
# Patterns to search for within files that indicate stage-specific writes
stage_patterns = ['normalized', 'processed', 'joined', 'ai']
# Allowed writers: etl_modules/*, etl_tools/*, make_scripts/migrate_layout.py, make_scripts/intake_zip.py, make_scripts/provenance_audit.py
allowed_writers = set([
    'etl_modules', 'etl_tools', 'make_scripts/migrate_layout.py', 'make_scripts/intake_zip.py', 'make_scripts/provenance_audit.py', 'make_scripts/clean_data_make.py', 'make_scripts/lint_layout.py', 'make_scripts/lint_deprecated_exports.py'
])
...
for p in root.rglob('*'):
    ...
    for stage in stage_patterns:
        # basic grep for writing to these directories
        if re.search(rf"data/etl/.+/{stage}", txt):
            # If the file is explicitly allowed, skip
            if rel in allowed_writers:
                continue
